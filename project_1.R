...
for (cancer in names(df_cancer_H2AFY)) {
  ...
  # Ensure enough data in both expression groups
  if (nrow(df_cancer[df_cancer$expression_group == "High", ]) > 0 &&
      nrow(df_cancer[df_cancer$expression_group == "Low", ]) > 0) {
    
    # Create survival object using survival_time and event columns
    surv_object <- Surv(as.numeric(df_cancer$survival_time), df_cancer$event)  # Ensure survival_time is numeric here too
